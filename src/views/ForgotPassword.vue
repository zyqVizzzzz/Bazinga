				...
				<div class="card-edge">
					<div class="card-face">
						...
						<div class="retro-title-box mb-4">
							...
							>
								<div class="text-xl">找回密码</div>
								<div class="title-decoration-right">
									<i class="bi bi-sd-card text-3xl"></i>
								</div>
							</h1>
						</div>
...
const sendVerificationCode = async () => {
	...
	try {
		await apiClient.post("/users/send-reset-code", {
	...
	try {
		...
		if (response.data.code === 200 || response.data.code === 0) {
			if (response.data.data.code === 0) {
				showToast({ message: response.data.data.data, type: "success" });
				router.push("/login");
			} else {
				showToast({ message: response.data.data.data, type: "error" });
			}
		} else {
			showToast({ message: "重置密码失败", type: "error" });
		}
	} catch (error) {
