<template>
	<div class="container w-full mx-auto my-10 p-4">
		<div class="flex editor-box">
			<div class="editor-container text-sm relative w-3/5">
				<div class="text-sm rounded shadow-editor" style="overflow-y: auto">
					<div id="editor" class="editorjs-container"></div>
				</div>
			</div>
			<div class="right-panel w-2/5">
				<div
					class="option-group flex justify-center space-x-4 py-4 border-b border-gray-100"
				>
...
const initEditorJS = async () => {
	console.log("Initializing EditorJS...", scriptJson.value);
...
function checkBoldText(content) {
	...
	// 在添加新知识点之前检查重叠
	const checkOverlap = (newWord, sceneId) => {
		let hasOverlap = false;
		let shouldAdd = true;

		// 检查现有知识点
		for (const [existingWord, knowledge] of currentKnowledge.value.entries()) {
			if (knowledge.scenes.has(sceneId)) {
				// 检查是否存在包含关系
				if (newWord.includes(existingWord)) {
					// 新词包含旧词，移除旧词
					currentKnowledge.value.delete(existingWord);
					existingBoldWords.delete(existingWord.toLowerCase());
					hasOverlap = true;
				} else if (existingWord.includes(newWord)) {
					// 旧词包含新词，不添加新词
					shouldAdd = false;
					hasOverlap = true;
					break;
	...
	content.blocks.forEach((block, blockIndex) => {
		...
		if (
			...
			boldWords.forEach((word) => {
				const { shouldAdd, hasOverlap } = checkOverlap(word, currentSceneId);
				if (shouldAdd) {
