	...
	try {
		...
		try {
			...
			if (res.data.code === 200) {
				generatedKnowledgeItems = res.data.data;
				generatedKnowledgeItems.forEach((knowledgeItem) => {
					// 添加到知识点集合
		...
		// 4. 从后向前遍历原文块，应用高亮和添加知识点块
		// 这样可以确保知识点只添加在最后一次出现的地方
		for (let i = originalBlocks.length - 1; i >= 0; i--) {
			const block = originalBlocks[i];
			const blockId = block.id || block.originalIndex;
			if (!blockId) continue;

			console.log(originalBlocks[i], generatedKnowledgeItems);
			// 找出与当前块相关的，且尚未处理过的知识点
			const blockKnowledge = generatedKnowledgeItems.filter(
				(k) =>
					block.text.toLowerCase().includes(k.word.toLowerCase()) &&
					!processedKnowledgeInScene.has(k.word)
			);
			...
			if (blockKnowledge.length > 0) {
				...
				// 标记这些知识点为已处理
				blockKnowledge.forEach((k) => processedKnowledgeInScene.add(k.word));
				...
				if (blockIndex >= 0) {
					...
					// 检查是否已有知识点块
					const existingKnowledgeIndex = currentSceneBlocks.value.findIndex(
						(b) => b.id === knowledgeId
					);

					if (existingKnowledgeIndex >= 0) {
						// 更新现有知识点块
						currentSceneBlocks.value[existingKnowledgeIndex].text =
							knowledgeBlock.text;
					} else {
						// 插入新的知识点块
						currentSceneBlocks.value.splice(insertIndex, 0, knowledgeBlock);
