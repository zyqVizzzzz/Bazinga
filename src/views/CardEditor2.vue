<template>
	<div class="container w-full mx-auto mt-10 pt-2">
		<div class="flex editor-box">
			<!-- 卡片编辑器 -->
			<div
				class="right-panel w-2/3 mx-auto"
				:class="{
					'pointer-events-none opacity-95': rightPanelLoading,
				}"
			>
				<!-- 加载指示器 -->
				<div
					v-if="rightPanelLoading"
					class="absolute inset-0 flex items-center justify-center bg-white/30 backdrop-blur-xs z-50"
				>
								...
								<div class="scene-content space-y-4">
									...
									<div class="original-text space-y-2">
										...
										>
											...
											<div
												class="text-sm p-2 rounded cursor-pointer text-left"
												:class="{
											...
											>
												<!-- @click="handleBlockSelect(index, block)" -->
												<!-- 编辑状态 -->
												<div v-if="block.isEditing" class="flex">
													<div
														contenteditable="true"
														class="flex-1 rounded focus:outline-none focus:border-primary min-h-[24px]"
														@blur="saveBlockEdit(block)"
														@input="(e) => (block.text = e.target.innerText)"
														@keydown.enter.prevent="
															(e) => !e.isComposing && handleEnterKey(block, e)
														"
														@keydown.backspace="
															(e) => handleBackspace(e, block, index)
														"
														@paste.prevent="handlePaste($event, block)"
														ref="editableDiv"
													></div>
												</div>
												<div v-else>
...
// 历史记录相关的状态
const blockHistory = ref(new Map()); // 存储每个块的历史记录
const blockHistoryPointer = ref(new Map()); // 存储每个块的历史指针
...
// 添加新行
const addNewBlock = () => {
	const blockId = `block_${currentScene.value.index}_${currentSceneBlocks.value.length}`;
	...
	blocksMap.value.set(blockId, newBlock);
	currentSceneBlocks.value.push(newBlock);

	if (currentScene.value && currentScene.value.blockIds) {
		currentScene.value.blockIds.push(blockId);
	}

	editingBlockId.value = blockId;
	hasUnsavedChanges.value = true;

	// 使用 nextTick 等待 DOM 更新后聚焦
	nextTick(() => {
...
const handleEnterKey = (block, event) => {
	// 保存当前块
	saveBlockEdit(block);
...
const handleBackspace = (event, block, index) => {
	...
	// 检查是否已经回退到开头，需要同时检查光标位置
	if (
		editableDiv.innerText.length === 1 &&
		...
		const hasKnowledge = blocksMap.value.has(knowledgeId);
		const hasTranslation = blocksMap.value.has(translationId);

		// 如果有关联的知识点或翻译，显示确认对话框
		if (hasKnowledge || hasTranslation) {
			const confirmMessage = `确定要删除这一行${
				hasTranslation ? "及其翻译" : ""
			}${hasKnowledge && hasTranslation ? "和" : ""}${
				hasKnowledge ? "知识点" : ""
			}吗？`;
		...
		// 从 blocksMap 中删除该块
		if (blockId) {
			blocksMap.value.delete(blockId);
		...
		// 从当前场景块数组中删除该块及其关联的知识点和翻译块
		let deleteCount = 1; // 至少删除当前块

		// 检查下一个块是否是翻译块
		if (
			index + 1 < currentSceneBlocks.value.length &&
			currentSceneBlocks.value[index + 1].isTranslated
		) {
			deleteCount++;
		}
		...
		// 检查翻译块后面是否是知识点块
		if (
			index + deleteCount < currentSceneBlocks.value.length &&
			currentSceneBlocks.value[index + deleteCount].isKnowledge
		) {
			deleteCount++;
		}

		// 删除块及其关联的翻译和知识点
		currentSceneBlocks.value.splice(index, deleteCount);

		// 找到上一个可编辑的块
		let prevIndex = index - 1;
		while (prevIndex >= 0) {
			const prevBlock = currentSceneBlocks.value[prevIndex];
			if (
				!prevBlock.isTitle &&
				!prevBlock.isTranslated &&
				!prevBlock.isKnowledge
			) {
				// 设置上一个块为编辑状态，但保留其内容
				prevBlock.isEditing = true;
				editingBlockId.value = prevBlock.id;

				// 等待 DOM 更新后设置光标位置
				nextTick(() => {
					const prevEditableDiv = document.querySelector(
						`[contenteditable="true"]`
					);
					if (prevEditableDiv) {
						// 设置原有内容
						prevEditableDiv.innerText = prevBlock.text;
						prevEditableDiv.focus();

						// 将光标移到文本末尾
						const range = document.createRange();
						const sel = window.getSelection();
						range.selectNodeContents(prevEditableDiv);
...
const handlePaste = (event, currentBlock) => {
	event.preventDefault();
	...
	// 第一段文本放入当前编辑块
	currentBlock.text = textBlocks[0];
	saveBlockEdit(currentBlock);
	...
	for (let i = 1; i < textBlocks.length; i++) {
		...
		const newBlock = {
			...
			editorId: blockId,
			originalIndex: blockId,
			isEditing: false, // 设置为 false，这样不会都处于编辑状态
		};
...
const saveBlockEdit = (block) => {
	...
	if (sceneKnowledge.length > 0) {
		block.displayText = highlightKnowledgeInText(newText, sceneKnowledge);
	} else {
		block.displayText = newText;
	}
	...
	if (blockId) {
		...
		if (originalBlock) {
			originalBlock.text = newText;
			originalBlock.displayText = block.displayText;
		}
	}
...
const initKnowledgeDisplay = async () => {
	try {
		...
		scenes.value.forEach((scene, sceneIndex) => {
			...
			// 在处理每个场景前重置已处理的知识点
			const processedKnowledgeInScene = new Set(); // 添加一个集合来跟踪场景中已处理的知识点
			...
			console.log("===== 处理场景 =====", sceneId);
			console.log("当前场景的所有块:", sceneBlocks);
			...
			console.log("该场景的所有知识点:", matchedKnowledge);
...
const updateCurrentScene = async () => {
	...
	const currentBlockIds = new Set(
		editorBlocks.map((block) => block.id).filter(Boolean)
	);
	for (const [blockId, block] of blocksMap.value.entries()) {
		if (
			!block.isTranslated &&
			!block.isKnowledge &&
			!currentBlockIds.has(block.editorId)
		) {
			...
			// 找到一个删除的块就可以跳出循环了，因为已经重新应用了所有高亮
			break;
		}
	}
...
const processedWords = ref(new Set());
const applyKnowledgeHighlight = (block, sceneIndex, knowledgeMap) => {
	...
	if (knowledgeMap && knowledgeMap.size > 0) {
		Array.from(knowledgeMap.values()).forEach((knowledge) => {
			if (knowledge.scenes && knowledge.scenes.has(sceneId)) {
				// 只添加尚未处理过的知识点
				if (!processedWords.value.has(knowledge.word)) {
					matchedKnowledge.push(knowledge);
				}
			}
		});
	...
	if (matchedKnowledge.length > 0) {
		...
		matchedKnowledge.forEach((k) => {
			if (block.text.toLowerCase().includes(k.word.toLowerCase())) {
				processedWords.value.add(k.word);
			}
		});
	}
...
const resetKnowledgeHighlight = () => {
	processedWords.value.clear();
};
...
const handleBlockClick = (block, index) => {
	...
	nextTick(() => {
		...
		if (editableDiv) {
			// 设置原有内容
			editableDiv.innerText = block.text;
			editableDiv.focus();
...
// 处理文本块点击
const handleBlockSelect = (index, block) => {
	if (block.isTitle || block.isTranslated || block.isKnowledge) {
...
const handleSceneChange = () => {
	...
	if (scenes.value.length > 0) {
		...
		if (currentSceneStructure && currentSceneStructure.blockIds) {
			...
			// 从后向前遍历当前场景的所有块ID
			for (let i = currentSceneStructure.blockIds.length - 1; i >= 0; i--) {
				const blockId = currentSceneStructure.blockIds[i];
				const originalBlock = blocksMap.value.get(blockId);
				...
				if (originalBlock) {
					...
					// 找出与当前块相关的，且尚未处理过的知识点
					const blockKnowledge = sceneKnowledge.filter(
						(k) =>
					...
					if (blockKnowledge.length > 0) {
						...
						// 从前面插入知识点块
						newSceneBlocks.unshift(JSON.parse(JSON.stringify(knowledgeBlock)));
					}
					...
					if (blocksMap.value.has(translationId)) {
						...
						newSceneBlocks.unshift(
							JSON.parse(JSON.stringify(translationBlock))
						);
					}

					// 添加原文块
					newSceneBlocks.unshift(clonedOriginalBlock);
				}
			}
		}
